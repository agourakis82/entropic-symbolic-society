...
def load_graph(path: Path) -> nx.Graph:
    ...
    if suffix == '.graphml':
        ...
        raise ValueError(f"Unsupported graph format: {suffix}. Use .graphml or .gpickle.")
...
def ccdf(values: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    ccdf_vals = 1.0 - np.searchsorted(sorted_v, unique_vals, side='right') / sorted_v.size
...
def main() -> None:
    ...
    parser.add_argument('--graph', type=Path, default=Path('NHB_Symbolic_Mainfold/results/word_network.graphml'),
    ...
    parser.add_argument('--basename', type=str, default='ext_S3_degree_distribution',
    ...
    axes[0].hist(degrees_hist, bins=args.bins, color=palette['bars'], edgecolor='black',
    ...
    # CCDF
    axes[1].loglog(unique_deg, ccdf_vals, marker='o', linestyle='none', markersize=3.5,
                   color=palette['points'], alpha=0.85)
    ...
    footer = ('Extended Data Fig. S3 | Degree distribution and CCDF of the SWOW‐EN semantic graph. '
