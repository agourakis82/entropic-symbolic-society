...
def main():
    ...
    ap.add_argument("--in", dest="inp", required=True, help="Input CSV (entropy_curvature_all.csv)")
    ap.add_argument("--out", dest="out", default="source_fig2_entropy_curvature.csv", help="Output CSV path")
    ap.add_argument("--select-beta", type=float, default=None, help="Optional: filter a single beta value (e.g., 1.0)")
    ap.add_argument("--rename-regimes", help="Optional: CSV with mapping 'from,to' to rename regime labels")
    args = ap.parse_args()
    ...
    if not required.issubset(df.columns):
        print(f"ERROR: input missing columns {required - set(df.columns)}", file=sys.stderr)
        sys.exit(2)
